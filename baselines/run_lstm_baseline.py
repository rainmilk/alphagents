...
def _build_portfolios(predictions: pd.DataFrame,
    ...
    Each day, select the top-N stocks by predicted forward return and weight
    them equally (1/n) via ``portfolio_method`` (default equal_weight), with the
    same single-stock / industry caps every other baseline uses.
    """
...
def run_lstm_baseline(
    ...
    forward_period: int = 1,  # FIXED at 1 day for ML daily predictors (see body; arg ignored)
    seed: int = 42,
    ...
    # ── ML daily predictor: forward horizon is FIXED at 1 day ────────
    # These end-to-end models predict the NEXT-day return for every test
    # day. This is intentionally decoupled from the factor-method
    # evolution.forward_period (default 10) used by MASE/FAMA, so the
    # forward_period argument is ignored on purpose.
    forward_period = 1
